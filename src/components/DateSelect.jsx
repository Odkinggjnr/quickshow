...
const DateSelect = ({ dateTime, id }) => {
  ...
  return (
    <div id='dateSelect' className='pt-30'>
          ...
          <div className='flex items-center gap-6 text-sm mt-5'><ChevronLeftIcon width={28} className='cursor-pointer' />
            <span className='grid grid-cols-3 md:flex flex-wrap md:max-w-lg'>
              {Object.keys(dateTime).map((date) => (
                <button key={date} className={`flex flex-col items-center justify-center h-14 w-14 aspect-square rounded cursor-pointer ${selected === date ? 'bg-primary text-white' : 'hover:bg-primary/20 bg-white/10'} transition`} onClick={() => setSelected(date)}>
